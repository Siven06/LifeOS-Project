const transactionsScreen = {
    ...
    app.innerHTML = `
      <div class="min-h-screen bg-background pb-32">
        <header class="bg-surface/80 backdrop-blur-xl border-b border-white/10 flex items-center px-5 h-16 sticky top-0 z-50 gap-4">
          ...
          <h1 class="text-headline-md font-semibold flex-1">History</h1>
        ...
        <main class="max-w-3xl mx-auto px-5 pt-6">
          <div class="relative mb-4">
            ...
            <input id="tx-search" type="text" placeholder="Search transactions" class="w-full bg-surface-container border border-white/10 rounded-xl pl-12 pr-4 py-3 text-on-surface focus:outline-none focus:border-primary-container/50 transition-colors text-body-md"/>
          </div>

          <div class="flex gap-2 mb-2 overflow-x-auto scrollbar-hide" id="filter-tabs">
            <button class="filter-btn active px-4 py-2 rounded-full text-label-sm bg-primary-container text-on-primary" data-filter="">All</button>
            <button class="filter-btn px-4 py-2 rounded-full text-label-sm bg-surface-container text-on-surface-variant hover:bg-surface-container-high transition-colors" data-filter="INCOME">Income</button>
            <button class="filter-btn px-4 py-2 rounded-full text-label-sm bg-surface-container text-on-surface-variant hover:bg-surface-container-high transition-colors" data-filter="EXPENSE">Expenses</button>
          </div>
          ...
          <div class="flex gap-2 mb-6 overflow-x-auto scrollbar-hide" id="category-tabs">
            ${['', 'FOOD', 'TRANSPORT', 'SHOPPING', 'ENTERTAINMENT', 'BILLS', 'HEALTH'].map(c =>
              `<button class="cat-filter px-3 py-1.5 rounded-full text-label-sm transition-colors ${!c ? 'bg-primary-container/20 text-primary-container' : 'bg-surface-container text-on-surface-variant hover:bg-surface-container-high'}" data-cat="${c}">${c || 'All'}</button>`
            ).join('')}
          </div>
  ...
  async loadTransactions() {
    ...
    try {
      ...
      list.innerHTML = `<p class="text-center text-error text-body-md py-12">Failed to load transactions</p>`;
    }
  },
  ...
  applyClientFilter() {
    ...
    if (filtered.length === 0) {
      list.innerHTML = `<p class="text-center text-on-surface-variant text-body-md py-12">No transactions found</p>`;
      return;
    }
        ...
        <div class="flex items-center gap-4">
          ...
          <div>
            <div class="flex items-center gap-2">
              ...
              ${t.status === 'PENDING' ? '<span class="text-label-sm text-secondary bg-secondary/10 px-2 py-0.5 rounded-full">Pending</span>' : ''}
            </div>
  ...
  exportCSV() {
    ...
    if (txs.length === 0) { toast.info('No transactions to export'); return; }
    const rows = [['Date','Type','Category','Description','Amount','Status']];
    txs.forEach(t => rows.push([t.date, t.type, t.category, `"${t.description}"`, t.amount, t.status || 'COMPLETED']));
    const csv = rows.map(r => r.join(',')).join('\n');
    const blob = new Blob([csv], { type: 'text/csv' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a'); a.href = url; a.download = 'finflow_transactions.csv'; a.click();
  ...
  exportPDF() {
    ...
    printWin.document.write(`<html><head><title>FinFlow Transactions</title><style>
    ...
    </style></head><body><h1>FinFlow Transactions</h1>
    <table><tr><th>Date</th><th>Type</th><th>Category</th><th>Description</th><th>Amount</th></tr>`);
