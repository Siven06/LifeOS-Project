const alertsScreen = {
    ...
    app.innerHTML = `
      <div class="min-h-screen bg-background pb-32">
        <header class="bg-surface/80 backdrop-blur-xl border-b border-white/10 flex items-center px-5 h-16 sticky top-0 z-50 gap-4">
          ...
          <h1 class="text-headline-md font-semibold flex-1">Alerts</h1>
          <button id="btn-dismiss-all" class="text-primary-container text-label-sm hover:underline">Dismiss All</button>
        </header>
  ...
  async loadAlerts() {
    ...
    try {
      ...
      if (alerts.length === 0) {
        list.innerHTML = `
          <div class="glass-card rounded-2xl p-8 text-center">
            ...
            <p class="text-body-md text-on-surface-variant">No alerts</p>
            <p class="text-label-sm text-on-surface-variant mt-1">You're all caught up!</p>
          </div>`;
        return;
      }
      ...
      list.innerHTML = `<p class="text-center text-error text-body-md py-12">Failed to load alerts</p>`;
    }
  },
